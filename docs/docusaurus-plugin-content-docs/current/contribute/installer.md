...
* Note - `<plugin_name>` is cases sensitive. Allowed values are stated [here](/docs/contribute/plugin/intro.md) in the `Rust Crate` column. The logic is that the release name should be the same.
* Note - It's the same as the WasmEdge version if not specified.
