...
**Developers can refer [here to upgrade to 0.10.0](/docs/embed-guide/c/reference/upgrade_to_0.10.0.md).**
...
1. Limit
    ...
    Developers can initialize the strut by assigning it's value, and the `Max` value is needed to be larger or equal to the `Min` value.
    The API `WasmEdge_LimitIsEqual()` is provided to compare with 2 `WasmEdge_Limit` struts.
...
2. Host registrations
    ...
    The details will be introduced in the [prersegistrations of VM context](#preregistrations).
