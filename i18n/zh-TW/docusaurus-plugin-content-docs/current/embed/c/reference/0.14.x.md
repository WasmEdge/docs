...
[WasmEdge C API](https://github.com/WasmEdge/WasmEdge/blob/master/include/api/wasmedge/wasmedge.h) 是用於存取 WasmEdge 執行環境 `0.14.1` 版本的介面。以下是使用 WasmEdge C API 的指南。

**開發者可參考[此處升級到 v0.15.0](upgrade_to_0.15.0)。**

## WasmEdge 安裝

### 下載與安裝

最簡單的 WasmEdge 安裝方式是執行以下指令。系統必須先具備 `git` 與 `wget` 作為前置條件。

```bash
curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install.sh | bash -s -- -v 0.14.1
```

更多詳細資訊,請參考 WasmEdge 的[安裝指南](../../../start/install.md#install)。

### 編譯原始碼

安裝完 WasmEdge 後,以下指南可協助你測試 WasmEdge C API 的可用性。
...
WasmEdge C API 自 `0.11.0` 版本起導入 SONAME 與 SOVERSION,以呈現不同 C API 版本間的相容性。

0.11.0 之前的版本皆未進行版本標示。請確保函式庫版本與所使用的 C API 版本相對應。
...
在本節中,我們將介紹 WasmEdge 共用函式庫的工具與相關概念。

### 版本

`Version` 相關 API 讓開發者可以查詢 WasmEdge 共用函式庫的版本。
...
`WasmEdge_LogSetErrorLevel()` 與 `WasmEdge_LogSetDebugLevel()` API 可將日誌系統設定為除錯等級或錯誤等級。預設情況下,設定為錯誤等級,並隱藏除錯資訊。

開發者也可以使用 `WasmEdge_LogOff()` API 來停用所有日誌輸出。
...
在 WasmEdge 中,開發者應透過 API 將值轉換為 `WasmEdge_Value` 物件,以對應到 WASM 引數或回傳值的值。透過這些 API,輸出的 `WasmEdge_Value` 物件將正確記錄值的類型與內容。
...
<!-- prettier-ignore -->
:::note
這個物件是設計用來取代 WasmEdge C API 的原始緩衝區作為輸入與輸出。我們建議開發者使用 `WasmEdge_Bytes` 相關 API 而非原始緩衝區,例如使用 `WasmEdge_LoaderParseFromBytes()` 取代 `WasmEdge_LoaderParseFromBuffer()`。
:::
...
`WasmEdge_String` 物件用於呼叫 WASM 函式時的實例名稱,或是用於尋找實例的上下文。
...
2. 將具有長度的緩衝區包裝成 `WasmEdge_String`。
...
`WasmEdge_Result` 物件用於指明執行狀態。WASM 執行相關的 API 會回傳 `WasmEdge_Result` 來表示狀態。
...
例如 `VM`、`Store`、`Function` 等物件,皆是由 `Context` 所組成。所有上下文都可以透過呼叫對應的建立 API 來建立,並應透過呼叫對應的刪除 API 來銷毀。開發者必須負責管理這些上下文以進行記憶體管理。
...
WASM 資料結構用於建立實例,或可由實例上下文進行查詢。實例建立的細節將在[實例](#instances)中介紹。

1. Limit

   `WasmEdge_Limit` 結構在標頭檔中的定義如下:
   ...
   開發者可透過指派其值來初始化此結構,並且 `Max` 值必須大於或等於 `Min` 值。`WasmEdge_LimitIsEqual()` API 可用於比較兩個 `WasmEdge_Limit` 結構。

2. Function type 上下文

   `Function Type` 上下文用於 `Function` 的建立、檢查 `Function` 實例的值類型,或從 VM 取得具有函式名稱的函式類型。開發者可使用 `Function Type` 上下文的 API 取得參數或回傳值類型的資訊。
...
6. Tag type 上下文
   ...
   此功能只有在開啟 `Exception Handling` 提案時才能使用。
...
7. Import type 上下文
   ...
   `Import Type` 上下文用於從 [AST 模組](#ast-module)取得匯入資訊。開發者可從 `Import Type` 上下文取得外部類型(`function`、`table`、`memory`、`tag` 或 `global`)、匯入模組名稱以及外部名稱。查詢 `Import Type` 上下文的細節將在 [AST 模組](#ast-module)中介紹。
...
8. Export type 上下文
   ...
   `Export Type` 上下文用於從 [AST 模組](#ast-module)取得匯出資訊。開發者可從 `Export Type` 上下文取得外部類型(`function`、`table`、`memory`、`tag` 或 `global`)以及外部名稱。查詢 `Export Type` 上下文的細節將在 [AST 模組](#ast-module)中介紹。
...
呼叫[非同步執行 API](#asynchronous-execution) 後,開發者會取得 `WasmEdge_Async` 物件。開發者擁有此物件,且應呼叫 `WasmEdge_AsyncDelete()` API 來銷毀它。
...
1. 等待非同步執行
   ...
   或者開發者可以等待設定的時間限制。若超過時間限制,開發者可選擇取消執行。對於 AOT 模式下的可中斷執行,開發者應透過 `WasmEdge_ConfigureCompilerSetInterruptible()` API 將 `TRUE` 設定進 AOT 編譯器的設定上下文。
...
2. 取得非同步執行的執行結果
   ...
   開發者可使用 `WasmEdge_AsyncGetReturnsLength()` API 取得回傳值清單的長度。此函式會阻塞並等待執行。若執行已結束,此函式會立即回傳長度。若執行失敗,此函式會回傳 `0`。此函式可幫助開發者建立緩衝區以取得回傳值。若開發者已知緩衝區長度,可跳過此函式並使用 `WasmEdge_AsyncGet()` API 取得結果。
   ...
   `WasmEdge_AsyncGet()` API 會阻塞並等待執行。若執行已結束,此函式會將回傳值填入緩衝區並立即回傳執行結果。
...
設定上下文 `WasmEdge_ConfigureContext` 管理 `Loader`、`Validator`、`Executor`、`VM` 與 `Compiler` 上下文的相關設定。開發者可調整提案、VM 主機預先註冊(例如 `WASI`)以及 AOT 編譯器選項等設定,然後套用 `Configure` 上下文以建立執行環境的上下文。

1. 提案

   WasmEdge 支援開啟或關閉 WebAssembly 提案。此設定在以 `Configure` 上下文建立的任何上下文中皆有效。
    ...
    * * GC (interpreter only)
    * * Multiple memories
    * * Relaxed SIMD (`0.14.1` or upper only)
    * * Threads
...
2. 主機註冊
   ...
   此 enum 的元素保留供未來的其他內建主機函式(例如 `wasi-socket`)使用。
   ...
   詳細資訊將在 [VM 上下文的預先註冊](#built-in-host-modules-and-plug-in-preregistrations)中介紹。
...
3. 最大記憶體頁數
   ...
   開發者可透過此設定限制記憶體實例的頁面大小。當 WASM 執行時擴展記憶體實例的頁面大小超過限制時,頁面擴展會失敗。此設定僅在 `Executor` 與 `VM` 上下文中有效。
...
4. 強制直譯器模式
   ...
   若開發者想強制以直譯器模式執行 WASM 檔案或經過 AOT 編譯的 WASM,可開啟此設定。
...
6. 統計選項
   ...
   統計選項設定執行環境與 AOT 編譯器中關於指令計數、成本量測與時間量測的行為。這些設定在 `Compiler`、`VM` 與 `Executor` 上下文中皆有效。
...
統計上下文 `WasmEdge_StatisticsContext` 提供執行環境中的指令計數器、成本累計與成本限制。

在使用統計功能前,必須先設定統計設定。否則呼叫統計相關 API 的回傳值將是未定義行為。

1. 指令計數器

   指令計數器可協助開發者進行 WASM 執行的效能分析。開發者可從 `VM` 上下文取得 `Statistics` 上下文,或建立新的 `Statistics` 上下文用於建立 `Executor`。詳細資訊將在後續章節介紹。
...
2. 成本表
   ...
   成本表用於依照指令的權重累計指令成本。開發者可設定成本表陣列(索引為指令的位元組碼值,值為指令的成本)到 `Statistics` 上下文中。若設定了成本限制值,當執行環境中超過成本限制時,執行會立即回傳 `cost limit exceeded` 錯誤。
...
在本節中,我們將介紹 `WasmEdge_VMContext` 物件的功能,並展示執行 WASM 函式的範例。

### 使用 VM 上下文執行 WASM 的範例

以下範例展示如何執行 WASM 以取得費氏數列。此範例使用 [fibonacci.wat](https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/examples/wasm/fibonacci.wat)。
...
1. 快速執行 WASM 函式
   ...
   假設 WASM 檔案 [`fibonacci.wasm`](https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/examples/wasm/fibonacci.wat) 已被複製到目前目錄,且 C 檔案 `test.c` 內容如下:
   ...
   接著你可以編譯並執行:(第 5 個費氏數在 0 起始索引下為 8)
...
2. 手動實例化並執行 WASM 函式
   ...
   除了上述範例外,開發者也可以使用 `VM` 上下文 API 逐步執行 WASM 函式:
   ...
   接著你可以編譯並執行:(第 10 個費氏數在 0 起始索引下為 89)
   ...
   `VM` 上下文在建立後的狀態為 `Inited`。在成功載入 WASM 後,狀態會變為 `Loaded`。在成功驗證 WASM 後,狀態會變為 `Validated`。在成功實例化 WASM 後,狀態會變為 `Instantiated`,開發者即可呼叫函式。開發者可在任何狀態下註冊 WASM 或模組實例,但必須再次實例化 WASM。開發者也可以在任何狀態下載入 WASM,且必須在呼叫函式之前驗證並實例化 WASM 模組。在 `Instantiated` 狀態下,開發者可再次實例化 WASM 模組以重置舊的 WASM 執行環境結構。

### 建立 VM

`VM` 建立 API 接受 `Configure` 上下文與 `Store` 上下文。若開發者只需要預設設定,可傳入 `NULL` 給建立 API。`Store` 上下文的細節將在 [Store](#store) 中介紹。
...
1. [WASI (WebAssembly System Interface)](https://github.com/WebAssembly/WASI)
   ...
   開發者可在 `Configure` 上下文中為 VM 開啟 WASI 支援。
   ...
   也可以透過 API 建立 WASI 模組實例。詳細資訊將在[主機函式](#host-functions)與[主機模組註冊](#host-module-registrations)中介紹。

2. 外掛

   若使用者已[透過安裝程式安裝 WasmEdge 外掛](../../../start/install.md#install-wasmedge-plug-ins-and-dependencies),預設外掛路徑中可能會有數個外掛。

   在使用外掛之前,開發者應[從路徑載入外掛](#load-plug-ins-from-paths)。

   `VM` 上下文在建立時會自動建立並註冊已載入外掛的模組。此外,以下主機模組會在外掛未載入時被模擬:
   ...
   當 WASM 想呼叫這些主機函式但對應的外掛未安裝時,WasmEdge 會印出錯誤訊息並回傳錯誤。
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,主機函式會組合成具有模組名稱的 `WasmEdge_ModuleInstanceContext` 物件作為主機模組。詳細資訊請參考 [WasmEdge 執行環境中的主機函式](#host-functions)。

在本章中,我們示範如何將主機模組註冊到 `VM` 上下文中。請注意,開發者應確保已註冊模組實例的可用性,並在不再使用時刪除模組實例。
...
在 WebAssembly 中,WASM 模組中的實例可以被匯出,並被其他 WASM 模組匯入。WasmEdge VM 提供 API 讓開發者可以註冊與匯出任何 WASM 模組,並執行已註冊 WASM 模組中的函式或主機函式。

1. 以已匯出的模組名稱註冊 WASM 模組

   除非模組實例已包含模組名稱,否則每個 WASM 模組在註冊時都應該有唯一的名稱。
...
2. 執行已註冊 WASM 模組中的函式
   ...
   接著你可以編譯並執行:(第 20 個費氏數在 0 起始索引下為 89)
...
1. 非同步快速執行 WASM 函式
   ...
   接著你可以編譯並執行:(第 20 個費氏數在 0 起始索引下為 10946)
...
2. 手動實例化並非同步執行 WASM 函式
   ...
   接著你可以編譯並執行:(第 25 個費氏數在 0 起始索引下為 121393)
...
有時候開發者可能會需要取得 WASM 執行環境的實例。`VM` 上下文提供 API 來取得實例。

1. Store

   若 `VM` 上下文建立時未指派 `Store` 上下文,`VM` 上下文會配置並擁有一個 `Store` 上下文。
   ...
   開發者也可以使用 `Store` 上下文建立 `VM` 上下文。在這種情況下,開發者應確保 `Store` 上下文的生命週期。`Store` 上下文 API 的細節請參考 [Store 上下文](#store)。
...
2. 列出已匯出的函式
   ...
   WASM 模組實例化後,開發者可使用 `WasmEdge_VMExecute()` API 呼叫已匯出的 WASM 函式。為此,開發者可能需要關於已匯出 WASM 函式清單的資訊。函式類型的細節請參考[執行環境中的實例](#instances)。
   ...
   接著你可以編譯並執行:(`fibonacci.wasm` 中唯一已匯出的函式是 `fib`)
   ...
   若開發者想取得已註冊 WASM 模組中的已匯出函式名稱,請從 `VM` 上下文取得 `Store` 上下文,並參考 [Store 上下文](#store) API 透過模組名稱列出已註冊的函式。

3. 取得函式類型

   `VM` 上下文提供 API 來透過函式名稱尋找函式類型。函式類型的細節請參考[執行環境中的實例](#instances)。
...
4. 取得作用中的模組
   ...
   WASM 模組實例化後,會實例化一個由 `VM` 上下文擁有的匿名模組。開發者可能需要取得它以取得模組之外的實例。然後開發者可以使用 `WasmEdge_VMGetActiveModule()` API 取得該匿名模組實例。模組實例 API 的細節請參考[模組實例](#instances)。
...
5. 列出與取得已註冊的模組
   ...
   要列出與取得 `VM` 上下文中已註冊的模組,除了存取 `VM` 的 `store` 上下文外,開發者可使用以下 API。
...
6. 取得組成元件
   ...
   `VM` 上下文由 `Loader`、`Validator` 與 `Executor` 上下文所組成。對於想使用這些上下文但不建立另一個實例的開發者,這些 API 可協助開發者從 `VM` 上下文取得它們。取得的上下文由 `VM` 上下文所擁有,開發者不應呼叫它們的刪除函式。
...
接著你可以編譯並執行:(第 18 個費氏數在 0 起始索引下為 4181)
...
`Loader` 上下文可從檔案或緩衝區載入 WASM 二進位。WASM 與來自 [WasmEdge AOT 編譯器](#wasmedge-aot-compiler)的編譯後 WASM 皆受支援。
...
`Validator` 上下文可驗證 WASM 模組。每個 WASM 模組在實例化前都應該經過驗證。
...
`Executor` 上下文是 WASM 與編譯後 WASM 的執行器。此物件應根據 `Store` 上下文進行運作。`Store` 上下文的細節請參考[下一章](#store)。

1. 將 `AST module` 實例化並註冊為命名的 `Module` 實例

   與在 `VM` 上下文中[註冊主機模組](#host-module-registrations)或[匯入 WASM 模組](#wasm-registrations-and-executions)相同,開發者可以將 `AST module` 上下文實例化為命名的 `Module` 實例,並將其註冊到 `Store` 上下文中。註冊後,產生的 `Module` 實例會以給定的模組名稱匯出到 `Store`,並可在實例化另一個模組時連結。

   `Module` 實例 API 的細節請參考[實例](#instances)。`Store` 上下文只是用於在實例化時搜尋與連結已匯出模組的連結器。開發者應在不再使用時刪除輸出的 `Module` 實例。當 `Module` 實例被刪除時,它會自動取消連結所有已連結的 `Store` 上下文。
...
2. 註冊現有的 `Module` 實例並匯出模組名稱
   ...
   除了實例化並註冊 `AST module` 上下文之外,開發者可以將現有的 `Module` 實例註冊到 store 中,並匯出模組名稱(模組名稱已存在於 `Module` 實例中)。此情境發生在開發者為主機函式建立 `Module` 實例並想註冊以進行連結時。主機函式在 `Module` 實例中建構的細節請參考[主機函式](#host-functions)。
...
3. 將 `AST module` 實例化為匿名的 `Module` 實例
   ...
   WASM 或編譯後的 WASM 模組在呼叫函式前應該先實例化。在實例化 WASM 模組前,請檢查[匯入區段](https://webassembly.github.io/spec/core/syntax/modules.html#syntax-import)以確保匯入已註冊到 `Store` 上下文中進行連結。
...
4. 呼叫函式
   ...
   在註冊或實例化並取得產生的 `Module` 實例後,開發者可從 `Module` 實例取得已匯出的 `Function` 實例以進行呼叫。`Module` 實例 API 的細節請參考[實例](#instances)。使用 `WasmEdge_ExecutorInvoke()` API 呼叫 `Function` 實例的範例請參考[上述範例](#wasm-execution-example-step-by-step)。

5. 非同步呼叫函式

   類似[在 VM 中非同步執行 WASM 函式](#asynchronous-execution),開發者也可以透過 `Executor` 上下文 API 非同步呼叫函式。

   取得 `Function` 實例後,開發者會透過呼叫 `WasmEdge_ExecutorAsyncInvoke()` API 取得 `Async` 上下文。請參考[非同步](#async)章節以使用此上下文取得結果。

### AST 模組

`AST Module` 上下文呈現從 WASM 檔案或緩衝區載入的結構。開發者會在從[載入器](#loader)載入 WASM 檔案或緩衝區後取得此物件。在實例化之前,開發者也可以查詢 `AST Module` 上下文的匯入與匯出。
...
作為將 WASM 檔案或緩衝區載入為 `AST Module` 的反向操作,`Loader` 上下文也提供序列化工具,可將 `AST Module` 序列化回 WASM 緩衝區。
...
[Store](https://webassembly.github.io/spec/core/exec/runtime.html#store) 是表示 WebAssembly 程式可操控的所有全域狀態的執行環境結構。WasmEdge 中的 `Store` 上下文是一個物件,作為連結器在實例化 WASM 模組時提供實例的匯出與匯入。開發者可從 `Store` 上下文取得已命名的模組,並應在不再使用時刪除已註冊到 `Store` 上下文的 `Module` 實例。

當 `Store` 上下文被刪除時,已連結的 `Module` 實例會自動取消連結至此 `Store` 上下文。當 `Module` 實例被刪除時,它會自動取消連結所有已連結的 `Store` 上下文。
...
實例是 WASM 的執行環境結構。開發者可從 `Store` 上下文取得 `Module` 實例,並從 `Module` 實例取得其他實例。單一實例可透過其建立函式配置。開發者可以將實例建構到 `Module` 實例中進行註冊。詳細資訊請參考[主機函式](#host-functions)。透過建立函式建立的實例應由開發者銷毀,除非它們已加入 `Module` 實例中。

1. 模組實例

   在實例化或註冊 `AST module` 上下文後,開發者會取得 `Module` 實例作為結果,並有責任在不再使用時銷毀它。也可以為主機模組建立 `Module` 實例。詳細資訊請參考[主機函式](#host-functions)。`Module` 實例提供 API 來列出與尋找模組中已匯出的實例。
...
2. 函式實例
   ...
   [主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,開發者可以為主機函式建立 `Function` 上下文,並將它們加入 `Module` 實例上下文以註冊到 `VM` 或 `Store`。開發者可透過 API 從 `Function` 上下文取得 `Function Type`。`Host Function` 指南的細節請參考[下一章](#host-functions)。
...
3. 表格實例
   ...
   在 WasmEdge 中,開發者可建立 `Table` 上下文並將其加入 `Module` 實例上下文以註冊到 `VM` 或 `Store`。`Table` 上下文提供 API 控制表格實例中的資料。
...
4. 記憶體實例
   ...
   在 WasmEdge 中,開發者可建立 `Memory` 上下文並將其加入 `Module` 實例上下文以註冊到 `VM` 或 `Store`。`Memory` 上下文提供 API 控制記憶體實例中的資料。
...
5. 標籤實例
   ...
   與其他實例不同,`Tag` 上下文僅在開啟 `Exception Handling` 提案時可用,並可從 `Module` 實例上下文取得。開發者可從實例取得 `Tag Type`。
...
6. 全域實例
   ...
   在 WasmEdge 中,開發者可建立 `Global` 上下文並將其加入 `Module` 實例上下文以註冊到 `VM` 或 `Store`。`Global` 上下文提供 API 控制全域實例中的值。
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,開發者可建立 `Function`、`Memory`、`Table` 與 `Global` 上下文,並將其加入 `Module` 實例上下文以註冊到 `VM` 或 `Store`。

1. 主機函式配置

   開發者可使用以下函式簽章定義 C 函式作為主機函式主體:
...
2. 呼叫框架上下文
   ...
   `WasmEdge_CallingFrameContext` 是讓開發者存取[呼叫堆疊頂端框架](https://webassembly.github.io/spec/core/exec/runtime.html#activations-and-frames)模組實例的上下文。根據 [WASM 規範](https://webassembly.github.io/spec/core/exec/instructions.html#function-calls),呼叫函式時會將具有模組實例的框架推入堆疊。因此,主機函式可存取頂端框架的模組實例以取得記憶體實例以讀寫資料。
   ...
   除了使用 `WasmEdge_CallingFrameGetMemoryInstance()` API 透過索引取得模組實例中的記憶體實例之外,開發者也可使用 `WasmEdge_CallingFrameGetModuleInstance()` 直接取得模組實例。因此開發者可透過 `WasmEdge_ModuleInstanceContext` API 取得已匯出的上下文。此外,開發者可以使用 `WasmEdge_CallingFrameGetExecutor()` API 取得目前使用的執行器上下文。

3. 主機函式的使用者自訂錯誤碼

   在主機函式中,WasmEdge 提供 `WasmEdge_Result_Success` 來回傳成功、`WasmEdge_Result_Terminate` 來終止 WASM 執行,以及 `WasmEdge_Result_Fail` 來回傳失敗。WasmEdge 也提供回傳使用者指定碼的用法。開發者可使用 `WasmEdge_ResultGen()` API 產生具有錯誤碼的 `WasmEdge_Result`,並使用 `WasmEdge_ResultGetCode()` API 取得錯誤碼。

   > 請注意:錯誤碼僅支援 24 位元整數(`uint32_t` 中的 0 ~ 16777216)。大於 24 位元的值會被截斷。
...
4. 建構具有主機實例的模組實例
   ...
   除了透過註冊或實例化 WASM 模組建立 `Module` 實例之外,開發者可以使用模組名稱建立 `Module` 實例,並將 `Function`、`Memory`、`Table` 與 `Global` 實例以其匯出名稱加入其中。
...
5. 指定的模組實例
   ...
   開發者可建立這些模組實例上下文,並將它們註冊到 `Store` 或 `VM` 上下文,而不需要調整 `Configure` 上下文的設定。
...
6. 範例
   ...
   接著你可以編譯並執行:(1234 + 5678 的結果為 6912)
...
7. 主機資料範例
   ...
   開發者可在 `Function` 上下文中設定外部資料物件,並在函式主體中存取該物件。假設來自 WAT 的簡單 WASM 如下:
...
8. 具有終結器的模組實例中的主機資料
   ...
   除了將主機資料設定到主機函式中之外,開發者可以將主機資料的擁有權連同其終結器設定並轉移到 `Module` 實例上下文中。這在實作外掛時可能會很有用。
...
WasmEdge 外掛是共用函式庫,提供 WasmEdge 執行環境載入並建立主機模組實例。透過外掛,WasmEdge 執行環境可以更輕易地擴充。
...
2. WasmEdge 安裝路徑下的 `../plugin/` 目錄。
3. 若 WasmEdge 安裝在系統目錄下(例如 `/usr` 與 `/usr/local`),則為函式庫路徑下的 `./wasmedge/` 目錄。
...
WasmEdge 以直譯器模式執行 WASM 檔案,且 WasmEdge 也支援 AOT(ahead-of-time)模式,無需修改任何程式碼即可執行。WasmEdge AOT(ahead-of-time)編譯器將 WASM 檔案編譯後即可在 AOT 模式下執行,比直譯器模式快得多。開發者可將 WASM 檔案編譯為共用函式庫格式或通用 WASM 格式的編譯後 WASM 檔案,以進行 AOT 模式執行。
...
接著你可以編譯並執行(輸出檔案為「fibonacci-aot.wasm」):
...
詳細資訊請參考 [AOT 編譯器選項設定](#configurations)。

## WasmEdge CLI 工具

在本節中,我們將介紹用於觸發 WasmEdge CLI 工具的 C API。

除了執行 `wasmedge` 與 `wasmedgec` CLI 工具之外,開發者可以透過 WasmEdge C API 觸發 WasmEdge CLI 工具。API 引數與 CLI 工具的命令列引數相同。

### 執行環境 CLI

`WasmEdge_Driver_Tool()` API 提供與執行 [`wasmedge run` 命令](../../../start/build-and-run/run.md)相同的功能。

請注意,此 API 呈現舊版的 `wasmedge` CLI 工具,與 `wasmedge run` 命令相同。對於目前統一的 `wasmedge` CLI,請參考[下方 API](#unified-cli)。
...
請注意,此 API 呈現舊版的 `wasmedgec` CLI 工具,與 `wasmedge compile` 命令相同。對於目前統一的 `wasmedge` CLI,請參考[下方 API](#unified-cli)。
...
在 Windows 平台上,開發者可使用 `WasmEdge_Driver_ArgvCreate()` 與 `WasmEdge_Driver_ArgvDelete()` API 來轉換與處理 `UTF-8` 命令列引數,或使用 `WasmEdge_Driver_SetConsoleOutputCPtoUTF8()` API 將主控台輸出代碼頁設定為 `UTF-8`。
