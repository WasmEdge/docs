...
[WasmEdge C API](https://github.com/WasmEdge/WasmEdge/blob/0.12.1/include/api/wasmedge/wasmedge.h) 表示用於存取 `0.12.1` 版本 WasmEdge 執行環境的介面。以下是使用 WasmEdge C API 的指南。

**開發者可參考[這裡升級至 0.13.0](upgrade_to_0.13.0.md)。**
...
- [WasmEdge VM](#wasmedge-vm)
  ...
  - [VM 建立](#vm-creations)
  - [內建主機模組與外掛預先註冊](#built-in-host-modules-and-plug-in-preregistrations)
...
| 自 0.12.0 起 | libwasmedge.so | libwasmedge.so.0 | libwasmedge.so.0.0.2 |
