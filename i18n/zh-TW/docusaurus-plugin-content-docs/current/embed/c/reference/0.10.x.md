...
[WasmEdge C API](https://github.com/WasmEdge/WasmEdge/blob/0.10.1/include/api/wasmedge/wasmedge.h) 表示用於存取 `0.10.1` 版本 WasmEdge 執行環境的介面。以下是使用 WasmEdge C API 的指南。

**開發者可參考[這裡升級至 0.11.0](upgrade_to_0.11.0.md)。**
...
除了執行 `wasmedge` 與 `wasmedgec` CLI 工具外,開發者也可以透過 WasmEdge C API 觸發 WasmEdge CLI 工具。API 引數與 CLI 工具的命令列引數相同。
...
2. 手動實例化並執行 WASM 函式
   ...
   `VM` 上下文建立時的狀態為 `Inited`。在成功載入 WASM 之後,狀態會變為 `Loaded`。在成功驗證 WASM 之後,狀態會變為 `Validated`。在成功實例化 WASM 之後,狀態會變為 `Instantiated`,開發者可以呼叫函式。開發者可以在任何狀態下註冊 WASM 或模組實例,但需要再次實例化 WASM。開發者也可以在任何狀態下載入 WASM,但需要先驗證並實例化 WASM 模組,才能呼叫函式。處於 `Instantiated` 狀態時,開發者可以再次實例化 WASM 模組以重置舊的 WASM 執行環境結構。
...
1. [WASI(WebAssembly System Interface)](https://github.com/WebAssembly/WASI)
   ...
   同樣地,也可以透過 API 建立 WASI 模組實例。細節將於[主機函式](#host-functions)與[主機模組註冊](#host-module-registrations)中介紹。

2. [WasmEdge_Process](https://crates.io/crates/wasmedge_process_interface)

   此預先註冊是為了 WasmEdge 在 `Rust` 原始碼上的程序介面。開啟此預先註冊後,VM 將支援 `wasmedge_process` 外掛。
   ...
   同樣地,也可以透過 API 建立 WasmEdge_Process 模組實例。細節將於[主機函式](#host-functions)與[主機模組註冊](#host-module-registrations)中介紹。

3. [WASI-NN 提案](https://github.com/WebAssembly/wasi-nn)(僅 `0.10.1` 或更高版本)

   開發者可以在 `Configure` 上下文中為 VM 開啟 WASI-NN 提案支援。

   > 注意:請檢查[相依性與先決條件](../../../start/install.md#wasi-nn-plug-in-with-openvino-backend)是否已滿足。
   ...
   同樣地,也可以透過 API 建立 WASI-NN 模組實例。細節將於[主機函式](#host-functions)與[主機模組註冊](#host-module-registrations)中介紹。

4. [WASI-Crypto 提案](https://github.com/WebAssembly/wasi-crypto)(僅 `0.10.1` 或更高版本)

   開發者可以在 `Configure` 上下文中為 VM 開啟 WASI-Crypto 提案支援。

   > 注意:請檢查[相依性與先決條件](../../../start/install.md#wasi-crypto-plug-in)是否已滿足。
   ...
   同樣地,也可以透過 API 建立 WASI-Crypto 模組實例。細節將於[主機函式](#host-functions)與[主機模組註冊](#host-module-registrations)中介紹。

### 主機模組註冊

[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,主機函式由具有模組名稱的 `WasmEdge_ModuleInstanceContext` 物件組成主機模組。詳細內容請參考 [WasmEdge 執行環境中的主機函式](#host-functions)。在本章節中,我們會展示將主機模組註冊到 `VM` 上下文中的範例。
...
1. 使用匯出模組名稱註冊 WASM 模組
   ...
   除非模組實例已包含模組名稱,否則每個 WASM 模組在註冊時都必須有唯一的名稱。假設將文字格式 [fibonacci.wat](https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/examples/wasm/fibonacci.wat) 的 WASM 檔案 `fibonacci.wasm` 複製到目前目錄。
...
4. 取得作用中模組
   ...
   WASM 模組實例化後,會實例化一個匿名模組並由 `VM` 上下文擁有。開發者可能需要取得它以取得模組之外的實例。然後開發者可以使用 `WasmEdge_VMGetActiveModule()` API 取得該匿名模組實例。關於模組實例 API 的細節,請參考[模組實例](#instances)。
...
1. 將 `AST module` 實例化並註冊為具名的 `Module` 實例

   就像在 `VM` 上下文中[註冊主機模組](#host-module-registrations)或[匯入 WASM 模組](#wasm-registrations-and-executions)一樣,開發者可以透過 `Executor` API 將 `AST module` 上下文實例化並註冊到 `Store` 上下文,作為具名的 `Module` 實例。註冊之後,產生的 `Module` 實例會以指定的模組名稱匯出,並在實例化另一個模組時可被連結。關於 `Module` 實例 API 的細節,請參考[實例](#instances)。
...
2. 註冊現有的 `Module` 實例並匯出模組名稱
   ...
   除了實例化並註冊 `AST module` 上下文之外,開發者可以將現有的 `Module` 實例註冊到 store 中,並匯出模組名稱(該名稱已存在於 `Module` 實例中)。這種情況發生在開發者為主機函式建立 `Module` 實例,並希望將其註冊以供連結時。關於在 `Module` 實例中建構主機函式的細節,請參考[主機函式](#host-functions)。
...
3. 將 `AST module` 實例化為匿名的 `Module` 實例
   ...
   WASM 或編譯 WASM 模組在函式呼叫之前應該先實例化。在實例化 WASM 模組之前,請先檢查[匯入段](https://webassembly.github.io/spec/core/syntax/modules.html#syntax-import),確保匯入已註冊到 `Store` 上下文中以供連結。
...
4. 呼叫函式
   ...
   註冊或實例化並取得結果的 `Module` 實例後,開發者可以從 `Module` 實例取得已匯出的 `Function` 實例以進行呼叫。關於 `Module` 實例 API 的細節,請參考[實例](#instances)。關於使用 `WasmEdge_ExecutorInvoke()` API 呼叫 `Function` 實例的內容,請參考[上方的範例](#wasm-execution-example-step-by-step)。
...
[Store](https://webassembly.github.io/spec/core/exec/runtime.html#store) 是執行環境結構,用於表示可被 WebAssembly 程式操作的所有全域狀態。WasmEdge 中的 `Store` 上下文是一個物件,在實例化 WASM 模組時提供實例匯出與匯入。開發者可以從 `Store` 上下文取得具名模組。
...
實例是 WASM 的執行環境結構。開發者可以從 `Store` 上下文取得 `Module` 實例,並從 `Module` 實例取得其他實例。單一實例可由其建立函式配置。開發者可以將實例建構到 `Module` 實例中以進行註冊。詳細內容請參考[主機函式](#host-functions)。由建立函式建立的實例應由開發者銷毀,除非它們已加入 `Module` 實例中。

1. 模組實例

   實例化或註冊 `AST module` 上下文後,開發者會取得 `Module` 實例作為結果,並有責任在不使用時銷毀它。`Module` 實例也可以為主機模組建立。詳細內容請參考[主機函式](#host-functions)。`Module` 實例提供 API 以列出並尋找模組中已匯出的實例。
...
2. 函式實例
   ...
   [主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,開發者可以為主機函式建立 `Function` 上下文,並將其加入 `Module` 實例上下文中,以便註冊到 `VM` 或 `Store`。開發者可以透過 API 從 `Function` 上下文取得 `Function Type`。關於 `Host Function` 指南的細節,請參考[下一章](#host-functions)。
...
3. 表格實例
   ...
   在 WasmEdge 中,開發者可以建立 `Table` 上下文,並將其加入 `Module` 實例上下文中,以便註冊到 `VM` 或 `Store`。`Table` 上下文提供 API 來控制表格實例中的資料。
...
4. 記憶體實例
   ...
   在 WasmEdge 中,開發者可以建立 `Memory` 上下文,並將其加入 `Module` 實例上下文中,以便註冊到 `VM` 或 `Store`。`Memory` 上下文提供 API 來控制記憶體實例中的資料。
...
5. 全域實例
   ...
   在 WasmEdge 中,開發者可以建立 `Global` 上下文,並將其加入 `Module` 實例上下文中,以便註冊到 `VM` 或 `Store`。`Global` 上下文提供 API 來控制全域實例中的值。
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,開發者可以建立 `Function`、`Memory`、`Table` 與 `Global` 上下文,並將其加入 `Module` 實例上下文中,以便註冊到 `VM` 或 `Store`。
...
2. 使用主機實例建構模組實例
   ...
   除了透過註冊或實例化 WASM 模組來建立 `Module` 實例之外,開發者可以使用模組名稱建立 `Module` 實例,並使用匯出名稱將 `Function`、`Memory`、`Table` 與 `Global` 實例加入其中。
...
3. 特定模組實例
   ...
   `WasmEdge_ModuleInstanceCreateWasiNN()` API 可為 `WASI-NN` 外掛建立並初始化 `wasi_ephemeral_nn` 模組實例(僅 `0.10.1` 或更高版本)。

   `WasmEdge_ModuleInstanceCreateWasiCryptoCommon()` API 可為 `WASI-Crypto` 外掛建立並初始化 `wasi_ephemeral_crypto_common` 模組實例(僅 `0.10.1` 或更高版本)。

   `WasmEdge_ModuleInstanceCreateWasiCryptoAsymmetricCommon()` API 可為 `WASI-Crypto` 外掛建立並初始化 `wasi_ephemeral_crypto_asymmetric_common` 模組實例(僅 `0.10.1` 或更高版本)。

   `WasmEdge_ModuleInstanceCreateWasiCryptoKx()` API 可為 `WASI-Crypto` 外掛建立並初始化 `wasi_ephemeral_crypto_kx` 模組實例(僅 `0.10.1` 或更高版本)。

   `WasmEdge_ModuleInstanceCreateWasiCryptoSignatures()` API 可為 `WASI-Crypto` 外掛建立並初始化 `wasi_ephemeral_crypto_signatures` 模組實例(僅 `0.10.1` 或更高版本)。

   `WasmEdge_ModuleInstanceCreateWasiCryptoSymmetric()` API 可為 `WASI-Crypto` 外掛建立並初始化 `wasi_ephemeral_crypto_symmetric` 模組實例(僅 `0.10.1` 或更高版本)。

   `WasmEdge_ModuleInstanceCreateWasmEdgeProcess()` API 可為 `wasmedge_process` 外掛建立並初始化 `wasmedge_process` 模組實例。

   開發者可以建立這些模組實例上下文,並將其註冊到 `Store` 或 `VM` 上下文,而非在 `Configure` 上下文中調整設定。

   > 注意:對於 `WASI-NN` 外掛,請檢查[相依性與先決條件](../../../start/install.md#wasi-nn-plug-in-with-openvino-backend)是否已滿足。注意:對於 `WASI-Crypto` 外掛,請檢查[相依性與先決條件](../../../start/install.md#wasi-crypto-plug-in)是否已滿足。建議將 5 個模組一起建立並註冊。
...
5. 主機資料範例
   ...
   開發者可以將外部資料物件設定到 `Function` 上下文,並在函式主體中存取該物件。假設有一個來自 WAT 的簡單 WASM 如下:
