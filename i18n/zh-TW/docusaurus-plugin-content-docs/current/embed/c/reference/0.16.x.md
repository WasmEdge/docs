...
[WasmEdge C API](https://github.com/WasmEdge/WasmEdge/blob/master/include/api/wasmedge/wasmedge.h) 代表存取 `0.16.3` 版本 WasmEdge 執行環境的介面。以下為使用 WasmEdge C API 的指南。

**開發者可參考[此處升級至 v0.17.0](upgrade_to_0.17.0)。**

## WasmEdge 安裝

### 下載並安裝

最簡便的 WasmEdge 安裝方式是執行以下指令。您的系統必須具備 `git` 與 `wget` 作為前置需求。

```bash
curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install.sh | bash -s -- -v 0.16.3
```

更多細節請參考 WasmEdge 安裝的[安裝指南](../../../start/install.md#install)。

### 編譯原始碼

安裝 WasmEdge 後,以下指南可協助您測試 WasmEdge C API 的可用性。

1. 準備測試 C 檔案(假設儲存為 `test.c`):
...
3. 執行並取得預期輸出。
   ...
   ```bash
   $ ./a.out
   WasmEdge version: 0.16.3
   ```

### ABI 相容性

WasmEdge C API 自 `0.11.0` 版本開始引入 SONAME 與 SOVERSION,以表示不同 C API 版本之間的相容性。

`0.11.0` 之前的版本皆未進行版本標示。請確保函式庫版本與您所使用的對應 C API 版本相同。

| WasmEdge 版本 | WasmEdge C API 函式庫名稱 | WasmEdge C API SONAME | WasmEdge C API SOVERSION |
| --- | --- | --- | --- |
| < 0.11.0 | libwasmedge_c.so | 未標示版本 | 未標示版本 |
...
在本節中,我們將介紹 WasmEdge 共用函式庫的工具與概念。

### 版本

`Version` 相關的 API 提供開發者檢查 WasmEdge 共用函式庫的版本。
...
1. 設定日誌等級
   ...
   `WasmEdge_LogSetErrorLevel()` 與 `WasmEdge_LogSetDebugLevel()` API 可將日誌系統設為除錯等級或錯誤等級。預設為錯誤等級,並隱藏除錯資訊。

   開發者可透過 `WasmEdge_LogSetLevel()` API 並搭配等級列舉設定日誌等級:
   ...
   開發者也可使用 `WasmEdge_LogOff()` API 停用所有日誌。

2. 日誌回呼

   為取得詳細的結果或錯誤訊息,WasmEdge 提供回呼介面,讓開發者註冊自訂回呼函式至日誌輸出端。
...
2. 參照類型:用於 `Reference-Types` 或 `Typed-Function References` 提案的 `funcref` 與 `externref`
...
在 WasmEdge 中,開發者應透過 API 將值轉換為 `WasmEdge_Value` 物件,以對應 WASM 引數或回傳值。透過這些 API,輸出的 `WasmEdge_Value` 物件將正確記錄值類型與值。
...
`WasmEdge_Bytes` 物件用於從記憶體載入或編譯模組的輸入緩衝區,或序列化模組的輸出緩衝區。

<!-- prettier-ignore -->
:::note
此物件用於取代 WasmEdge C API 中作為輸入與輸出的原始緩衝區。我們建議開發者使用 `WasmEdge_Bytes` 相關 API 而非原始緩衝區,例如使用 `WasmEdge_LoaderParseFromBytes()` 取代 `WasmEdge_LoaderParseFromBuffer()`。
:::
...
1. 從 C 字串(帶有 NULL 結尾的 `const char *`)或帶有長度的緩衝區建立 `WasmEdge_String`。

   C 字串或緩衝區的內容將被複製到 `WasmEdge_String` 物件中。
...
`WasmEdge_Result` 物件指定執行狀態。關於 WASM 執行的 API 會回傳 `WasmEdge_Result` 以表示狀態。
...
`VM`、`Store` 與 `Function` 等物件由 `Context` 組成。所有上下文皆可透過對應的建立 API 建立,並應透過對應的刪除 API 銷毀。開發者有責任管理上下文以進行記憶體管理。
...
WASM 資料結構用於建立實例,或可從實例上下文中查詢。實例建立的細節將在 [Instances](#instances) 中介紹。
...
1. Limit
   ...
   開發者可透過指定其值來初始化結構,且 `Max` 值必須大於或等於 `Min` 值。`WasmEdge_LimitIsEqual()` API 提供比較 2 個 `WasmEdge_Limit` 結構的功能。

2. 函式類型上下文

   `Function Type` 上下文用於建立 `Function`、檢查 `Function` 實例的值類型,或從 VM 中以函式名稱取得函式類型。開發者可使用 `Function Type` 上下文 API 取得參數或回傳值類型的資訊。
...
6. 標記類型上下文
   ...
   此功能僅在開啟 `Exception Handling` 提案時可用。
...
7. 匯入類型上下文
   ...
   `Import Type` 上下文用於從 [AST 模組](#ast-module)取得匯入資訊。開發者可從 `Import Type` 上下文取得外部類型(`function`、`table`、`memory`、`tag` 或 `global`)、匯入模組名稱與外部名稱。關於查詢 `Import Type` 上下文的細節將在 [AST 模組](#ast-module)中介紹。
...
8. 匯出類型上下文
   ...
   `Export Type` 上下文用於從 [AST 模組](#ast-module)取得匯出資訊。開發者可從 `Export Type` 上下文取得外部類型(`function`、`table`、`memory`、`tag` 或 `global`)與外部名稱。關於查詢 `Export Type` 上下文的細節將在 [AST 模組](#ast-module)中介紹。
...
呼叫[非同步執行 API](#asynchronous-execution) 後,開發者將取得 `WasmEdge_Async` 物件。開發者擁有此物件,並應呼叫 `WasmEdge_AsyncDelete()` API 來銷毀它。
...
1. 等待非同步執行
   ...
   或者開發者可等待一段時間限制。若超過時間限制,開發者可選擇取消執行。對於 AOT 模式下的可中斷執行,開發者應透過 `WasmEdge_ConfigureCompilerSetInterruptible()` API 將 `TRUE` 設定至 AOT 編譯器的設定上下文中。
...
2. 取得非同步執行的執行結果
   ...
   開發者可使用 `WasmEdge_AsyncGetReturnsLength()` API 取得回傳值列表的長度。此函式會阻塞並等待執行完成。若執行已完成,此函式會立即回傳長度。若執行失敗,此函式會回傳 `0`。此函式可協助開發者建立緩衝區以取得回傳值。若開發者已知緩衝區長度,可跳過此函式並使用 `WasmEdge_AsyncGet()` API 取得結果。
   ...
   `WasmEdge_AsyncGet()` API 會阻塞並等待執行完成。若執行已完成,此函式會將回傳值填入緩衝區並立即回傳執行結果。
...
設定上下文 `WasmEdge_ConfigureContext` 管理 `Loader`、`Validator`、`Executor`、`VM` 與 `Compiler` 上下文的設定。開發者可調整提案、VM 主機預先註冊(例如 `WASI`)、AOT 編譯器選項等設定,然後套用 `Configure` 上下文以建立執行環境上下文。

1. 標準

   WasmEdge 支援自最早的 `1.0` 版本起的 WebAssembly 標準。開發者可使用 `WasmEdge_ConfigureSetWASMStandard()` API 在 `Configure` 上下文中指定 WASM 標準。指定 WASM 標準後,對應的 WASM 提案將會被啟用或停用,且所有提案設定將被覆寫。
...
2. 提案
   ...
   WasmEdge 支援開啟或關閉 WebAssembly 提案。此設定在任何透過 `Configure` 上下文建立的上下文中皆有效。
...
3. 主機註冊
   ...
   此列舉的元素保留供未來的其他內建主機函式(例如 `wasi-socket`)使用。
...
4. 最大記憶體頁數
   ...
   開發者可透過此設定限制記憶體實例的頁面大小。在 WASM 執行中擴展記憶體實例的頁面大小並超過限制大小時,頁面成長將會失敗。此設定僅在 `Executor` 與 `VM` 上下文中有效。
...
5. 強制直譯器模式
   ...
   若開發者想要強制以直譯器模式執行 WASM 檔案或已 AOT 編譯的 WASM,可開啟此設定。
...
6. AOT 編譯器選項
   ...
   AOT 編譯器選項設定關於最佳化等級、輸出格式、傾印 IR 與通用二進位的行為。
...
7. 統計選項
   ...
   統計選項設定執行環境與 AOT 編譯器中關於指令計數、成本測量與時間測量的行為。這些設定在 `Compiler`、`VM` 與 `Executor` 上下文中皆有效。
...
統計上下文 `WasmEdge_StatisticsContext` 提供執行環境中的指令計數器、成本加總與成本限制功能。

在使用統計之前,必須設定統計設定。否則,呼叫統計的回傳值是未定義行為。

1. 指令計數器

   指令計數器可協助開發者剖析 WASM 執行的效能。開發者可從 `VM` 上下文擷取 `Statistics` 上下文,或為了建立 `Executor` 而新建一個。細節將在下一段中介紹。
...
2. 成本表格
   ...
   成本表格用於累計帶有權重的指令成本。開發者可將成本表格陣列(索引為指令的位元組碼值,值為指令的成本)設定至 `Statistics` 上下文中。若設定了成本限制值,當執行環境中超過成本限制時,執行會立即回傳 `cost limit exceeded` 錯誤。
...
在本節中,我們將介紹 `WasmEdge_VMContext` 物件的函式,並展示執行 WASM 函式的範例。

### 使用 VM 上下文執行 WASM 的範例

以下展示執行 WASM 取得 Fibonacci 數的範例。此範例使用 [fibonacci.wat](https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/examples/wasm/fibonacci.wat)。

<!-- prettier-ignore -->
:::note
`fibonacci.wat` 檔案是以文字格式提供的。使用者應透過 [WABT 工具](https://github.com/WebAssembly/wabt) 將其轉換為對應的 WASM 二進位格式。
:::
...
1. 快速執行 WASM 函式
   ...
   假設 WASM 檔案 [`fibonacci.wasm`](https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/examples/wasm/fibonacci.wat) 已複製到目前的目錄,且 C 檔案 `test.c` 內容如下:
   ...
   然後您可以編譯並執行:(第 5 個 Fibonacci 數在 0 起始的索引中為 8)
...
2. 手動實例化並執行 WASM 函式
   ...
   除了上述範例外,開發者可使用 `VM` 上下文 API 逐步執行 WASM 函式:
   ...
   然後您可以編譯並執行:(第 10 個 Fibonacci 數在 0 起始的索引中為 89)
   ...
   `VM` 上下文建立時的狀態會是 `Inited`。成功載入 WASM 後,狀態會變為 `Loaded`。成功驗證 WASM 後,狀態會變為 `Validated`。成功實例化 WASM 後,狀態會變為 `Instantiated`,然後開發者就可以呼叫函式。開發者可在任何狀態下註冊 WASM 或模組實例,但他們必須再次實例化 WASM。開發者也可在任何狀態下載入 WASM,而他們必須在函式呼叫前驗證並實例化 WASM 模組。在 `Instantiated` 狀態下,開發者可再次實例化 WASM 模組以重設舊有的 WASM 執行環境結構。

### VM 建立

`VM` 建立 API 接受 `Configure` 上下文與 `Store` 上下文。若開發者僅需預設設定,只需傳遞 `NULL` 至建立 API。`Store` 上下文的細節將在 [Store](#store) 中介紹。
...
1. [WASI (WebAssembly System Interface)](https://github.com/WebAssembly/WASI)
   ...
   也可透過 API 建立 WASI 模組實例。細節將在[主機函式](#host-functions)與[主機模組註冊](#host-module-registrations)中介紹。

2. 外掛

   若使用者[透過安裝程式安裝 WasmEdge 外掛](../../../start/install.md#install-wasmedge-plug-ins-and-dependencies),則預設外掛路徑中可能會有多個外掛。

   在使用外掛之前,開發者應[從路徑載入外掛](#load-plug-ins-from-paths)。

   `VM` 上下文在建立時會自動建立並註冊已載入外掛的模組。此外,若外掛未載入,以下主機模組將會被模擬:
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,以匯入方式傳遞給 WASM 模組。在 WasmEdge 中,主機函式被組合成主機模組,作為帶有模組名稱的 `WasmEdge_ModuleInstanceContext` 物件。細節請參考 [WasmEdge 執行環境中的主機函式](#host-functions)。

在本章中,我們展示將主機模組註冊至 `VM` 上下文的範例。請注意,開發者應確保已註冊模組實例的可用性,並在不再使用時刪除模組實例。
...
在 WebAssembly 中,WASM 模組中的實例可以被匯出,也可以被其他 WASM 模組匯入。WasmEdge VM 提供 API 讓開發者註冊並匯出任何 WASM 模組,並執行已註冊 WASM 模組中的函式或主機函式。

1. 以匯出的模組名稱註冊 WASM 模組

   除非模組實例已包含模組名稱,每個 WASM 模組在註冊時都應該被唯一命名。
...
2. 執行已註冊 WASM 模組中的函式
   ...
   然後您可以編譯並執行:(第 20 個 Fibonacci 數在 0 起始的索引中為 89)
...
3. 強制刪除已註冊的 WASM 模組
   ...
   對於 VM 上下文中已實例化並註冊的模組,開發者可使用 `WasmEdge_VMForceDeleteRegisteredModule()` API 以名稱強制刪除並取消註冊模組實例。

   <!-- prettier-ignore -->
   :::note
   此 API 不會檢查模組實例的匯出與匯入相依性。開發者在使用此 API 時應自行確保模組相依性。未來將提供更安全的 API。
   :::
...
1. 非同步快速執行 WASM 函式
   ...
   然後您可以編譯並執行:(第 20 個 Fibonacci 數在 0 起始的索引中為 10946)
...
2. 手動實例化並非同步執行 WASM 函式
   ...
   然後您可以編譯並執行:(第 25 個 Fibonacci 數在 0 起始的索引中為 121393)
...
有時開發者可能需要取得 WASM 執行環境的實例。`VM` 上下文提供擷取實例的 API。

1. Store

   若 `VM` 上下文建立時未指定 `Store` 上下文,`VM` 上下文會配置並擁有一個 `Store` 上下文。
   ...
   開發者也可使用 `Store` 上下文建立 `VM` 上下文。在這種情況下,開發者應確保 `Store` 上下文的生命週期。關於 `Store` 上下文 API 的細節,請參考 [Store 上下文](#store)。
...
2. 列出匯出的函式
   ...
   在 WASM 模組實例化之後,開發者可使用 `WasmEdge_VMExecute()` API 呼叫已匯出的 WASM 函式。為此,開發者可能需要關於已匯出 WASM 函式列表的資訊。關於函式類型的細節,請參考[執行環境中的實例](#instances)。
   ...
   然後您可以編譯並執行:(`fibonacci.wasm` 中唯一匯出的函式是 `fib`)
   ...
   若開發者想取得已註冊 WASM 模組中匯出的函式名稱,請從 `VM` 上下文中擷取 `Store` 上下文,並參考 [Store 上下文](#store)的 API,以模組名稱列出已註冊的函式。

3. 取得函式類型

   `VM` 上下文提供 API 以函式名稱尋找函式類型。關於函式類型的細節,請參考[執行環境中的實例](#instances)。
...
4. 取得目前的模組
   ...
   在 WASM 模組實例化後,會實例化一個由 `VM` 上下文擁有的匿名模組。開發者可能需要擷取它,以取得模組以外的實例。然後開發者可使用 `WasmEdge_VMGetActiveModule()` API 來取得該匿名模組實例。關於模組實例 API 的細節,請參考[模組實例](#instances)。
...
5. 列出並取得已註冊的模組
   ...
   要列出並擷取 `VM` 上下文中已註冊的模組,除了存取 `VM` 的 `store` 上下文之外,開發者可使用以下 API。
...
6. 取得元件
   ...
   `VM` 上下文由 `Loader`、`Validator` 與 `Executor` 上下文組成。對於想使用這些上下文但不想另行建立實例的開發者,以下 API 可協助開發者從 `VM` 上下文中取得它們。取得的上下文由 `VM` 上下文所擁有,開發者不應呼叫它們的刪除函式。
...
除了透過 [`VM` 上下文](#wasmedge-vm)執行 WASM 之外,開發者可使用 `Loader`、`Validator`、`Executor` 與 `Store` 上下文逐步執行 WASM 函式或實例化 WASM 模組。
...
然後您可以編譯並執行:(第 18 個 Fibonacci 數在 0 起始的索引中為 4181)
...
`Loader` 上下文從檔案或緩衝區載入 WASM 二進位檔。WASM 與從 [WasmEdge AOT 編譯器](#wasmedge-aot-compiler)輸出的編譯後 WASM 皆有支援。
...
`Executor` 上下文是 WASM 與編譯後 WASM 的執行器。此物件應以 `Store` 上下文為基礎運作。關於 `Store` 上下文的細節,請參考[下一章](#store)。

1. 將 `AST 模組`實例化並註冊為具名 `Module` 實例

   如同[註冊主機模組](#host-module-registrations)或[在 `VM` 上下文中匯入 WASM 模組](#wasm-registrations-and-executions)一樣,開發者可將 `AST 模組`上下文實例化為具名 `Module` 實例,並將其註冊至 `Store` 上下文中。註冊之後,產生的 `Module` 實例會以給定的模組名稱匯出至 `Store`,並可於實例化另一個模組時連結。

   關於 `Module` 實例 API 的細節,請參考 [Instances](#instances)。`Store` 上下文僅作為實例化時搜尋與連結匯出模組的連結器。開發者應在輸出的 `Module` 實例不再使用時刪除它。當 `Module` 實例被刪除時,它會自動取消連結到所有已連結的 `Store` 上下文。
...
2. 註冊現有的 `Module` 實例並匯出模組名稱

   除了實例化並註冊 `AST 模組`上下文之外,開發者可將現有的 `Module` 實例註冊至 store 中,並匯出模組名稱(該名稱已存在於 `Module` 實例中)。這種情況發生在開發者為主機函式建立 `Module` 實例,並想要註冊它以進行連結時。關於在 `Module` 實例中建構主機函式的細節,請參考[主機函式](#host-functions)。
...
3. 將 `AST 模組`實例化為匿名 `Module` 實例

   WASM 或編譯後 WASM 模組在函式呼叫之前應該被實例化。在實例化 WASM 模組之前,請檢查[匯入區段](https://webassembly.github.io/spec/core/syntax/modules.html#syntax-import)以確保匯入已被註冊至 `Store` 上下文中進行連結。
...
4. 呼叫函式
   ...
   註冊或實例化並取得產生的 `Module` 實例後,開發者可從 `Module` 實例擷取匯出的 `Function` 實例以進行呼叫。關於 `Module` 實例 API 的細節,請參考 [Instances](#instances)。關於使用 `WasmEdge_ExecutorInvoke()` API 呼叫 `Function` 實例,請參考[上述範例](#wasm-execution-example-step-by-step)。

5. 非同步呼叫函式

   如同[在 VM 中非同步執行 WASM 函式](#asynchronous-execution),開發者也可透過 `Executor` 上下文 API 非同步呼叫函式。

   取得 `Function` 實例後,開發者可透過呼叫 `WasmEdge_ExecutorAsyncInvoke()` API 取得 `Async` 上下文。請參考 [Async](#async) 章節以使用此上下文取得結果。

### AST 模組

`AST Module` 上下文呈現從 WASM 檔案或緩衝區載入的結構。開發者在從[載入器](#loader)載入 WASM 檔案或緩衝區後會取得此物件。在實例化之前,開發者也可查詢 `AST Module` 上下文的匯入與匯出。
...
作為將 WASM 檔案或緩衝區載入 `AST Module` 的逆向操作,`Loader` 上下文也提供序列化器,可將 `AST Module` 序列化回 WASM 緩衝區。
...
[Store](https://webassembly.github.io/spec/core/exec/runtime.html#store) 是用於表示所有可被 WebAssembly 程式操作的全域狀態的執行環境結構。WasmEdge 中的 `Store` 上下文是作為連結器的物件,在實例化 WASM 模組時提供實例的匯出與匯入。開發者可從 `Store` 上下文擷取具名模組,且若不再使用,應刪除已註冊至 `Store` 上下文的 `Module` 實例。

當 `Store` 上下文被刪除時,已連結的 `Module` 實例會自動取消連結至此 `Store` 上下文。當 `Module` 實例被刪除時,它會自動取消連結至所有已連結的 `Store` 上下文。
...
實例是 WASM 的執行環境結構。開發者可從 `Store` 上下文擷取 `Module` 實例,並從 `Module` 實例擷取其他實例。單一實例可透過其建立函式配置。開發者可將實例建構至 `Module` 實例中以進行註冊。細節請參考[主機函式](#host-functions)。透過建立函式建立的實例應由開發者銷毀,除非它們已加入 `Module` 實例。

1. 模組實例

   在實例化或註冊 `AST 模組`上下文後,開發者會取得 `Module` 實例作為結果,並有責任在不使用時銷毀它。`Module` 實例也可為主機模組建立。細節請參考[主機函式](#host-functions)。`Module` 實例提供 API 來列出與尋找模組中匯出的實例。
...
2. 函式實例
   ...
   [主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,以匯入方式傳遞給 WASM 模組。在 WasmEdge 中,開發者可為主機函式建立 `Function` 上下文,並將其加入 `Module` 實例上下文,以便註冊至 `VM` 或 `Store` 中。開發者可透過 API 從 `Function` 上下文擷取 `Function Type`。關於`主機函式`指南的細節,請參考[下一章](#host-functions)。
...
3. 表格實例
   ...
   在 WasmEdge 中,開發者可建立 `Table` 上下文,並將其加入 `Module` 實例上下文,以便註冊至 `VM` 或 `Store` 中。`Table` 上下文提供 API 以控制表格實例中的資料。
...
4. 記憶體實例
   ...
   在 WasmEdge 中,開發者可建立 `Memory` 上下文,並將其加入 `Module` 實例上下文,以便註冊至 `VM` 或 `Store` 中。`Memory` 上下文提供 API 以控制記憶體實例中的資料。
...
5. 標記實例
   ...
   與其他實例不同,`Tag` 上下文僅在開啟 `Exception Handling` 提案時可用,且僅能從 `Module` 實例上下文擷取。開發者可從實例擷取 `Tag Type`。
...
6. 全域實例
   ...
   在 WasmEdge 中,開發者可建立 `Global` 上下文,並將其加入 `Module` 實例上下文,以便註冊至 `VM` 或 `Store` 中。`Global` 上下文提供 API 以控制全域實例中的值。
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,以匯入方式傳遞給 WASM 模組。在 WasmEdge 中,開發者可建立 `Function`、`Memory`、`Table` 與 `Global` 上下文,並將其加入 `Module` 實例上下文,以便註冊至 `VM` 或 `Store` 中。

1. 主機函式配置

   開發者可以下列函式簽章定義 C 函式作為主機函式的主體:
   ...
   接著,開發者可使用主機函式主體與函式類型建立 `Function` 上下文:
...
2. 呼叫框架上下文
   ...
   `WasmEdge_CallingFrameContext` 是讓開發者存取[呼叫堆疊頂端的框架](https://webassembly.github.io/spec/core/exec/runtime.html#activations-and-frames)的模組實例的上下文。根據 [WASM 規範](https://webassembly.github.io/spec/core/exec/instructions.html#function-calls),呼叫函式時會將帶有模組實例的框架推入堆疊。因此,主機函式可存取堆疊頂端框架的模組實例,以擷取記憶體實例進行資料讀寫。
   ...
   除了使用 `WasmEdge_CallingFrameGetMemoryInstance()` API 以模組實例中的索引取得記憶體實例之外,開發者可使用 `WasmEdge_CallingFrameGetModuleInstance()` 直接取得模組實例。因此,開發者可透過 `WasmEdge_ModuleInstanceContext` API 擷取匯出的上下文。另外,開發者可使用 `WasmEdge_CallingFrameGetExecutor()` API 取得目前使用的執行器上下文。

3. 主機函式的使用者自訂錯誤碼

   在主機函式中,WasmEdge 提供 `WasmEdge_Result_Success` 以回傳成功,`WasmEdge_Result_Terminate` 以終止 WASM 執行,以及 `WasmEdge_Result_Fail` 以回傳失敗。WasmEdge 也提供回傳使用者指定錯誤碼的用法。開發者可使用 `WasmEdge_ResultGen()` API 以錯誤碼產生 `WasmEdge_Result`,並使用 `WasmEdge_ResultGetCode()` API 取得錯誤碼。

   > 注意:錯誤碼僅支援 24 位元整數(在 `uint32_t` 中為 0 ~ 16777216)。大於 24 位元的值會被截斷。

   假設一個簡單的 WAT 形式 WASM 如下:
...
4. 使用主機實例建構模組實例
   ...
   除了透過註冊或實例化 WASM 模組建立 `Module` 實例之外,開發者可使用模組名稱建立 `Module` 實例,並將 `Function`、`Memory`、`Table` 與 `Global` 實例以其匯出名稱加入其中。
...
5. 指定的模組實例
   ...
   開發者可建立這些模組實例上下文並將其註冊至 `Store` 或 `VM` 上下文,而非調整 `Configure` 上下文中的設定。
...
6. 範例
   ...
   然後您可以編譯並執行:(1234 + 5678 的結果為 6912)
...
7. 主機資料範例
   ...
   開發者可將外部資料物件設定至 `Function` 上下文,並在函式主體中存取該物件。假設一個簡單的 WAT 形式 WASM 如下:
...
8. 帶有終結器的模組實例中的主機資料
   ...
   除了將主機資料設定至主機函式之外,開發者可將主機資料的所有權設定並轉移至 `Module` 實例上下文及其終結器。在實作外掛時可能會很有用。
...
WasmEdge 外掛是讓 WasmEdge 執行環境載入與建立主機模組實例的共用函式庫。透過外掛,WasmEdge 執行環境可被更輕易地擴充。
...
1. 環境變數 `WASMEDGE_PLUGIN_PATH` 指定的路徑。
2. 相對於 WasmEdge 安裝路徑的 `../plugin/` 目錄。
3. 若 WasmEdge 安裝於系統目錄下(例如 `/usr` 與 `/usr/local`),則為函式庫路徑下的 `./wasmedge/` 目錄。

要從特定路徑或特定目錄下載入外掛,開發者可使用此 API:
...
在本節中,我們將介紹 WasmEdge AOT 編譯器與選項。

WasmEdge 以直譯器模式執行 WASM 檔案,且 WasmEdge 也支援在不修改任何程式碼的情況下以 AOT(ahead-of-time)模式執行。WasmEdge AOT(ahead-of-time)編譯器將 WASM 檔案編譯,以便在比直譯器模式快得多的 AOT 模式下執行。開發者可將 WASM 檔案編譯為共用函式庫格式或通用 WASM 格式的編譯後 WASM 檔案,用於 AOT 模式執行。
...
然後您可以編譯並執行(輸出檔案為「fibonacci-aot.wasm」):
...
請注意,此 API 代表舊的 `wasmedge` CLI 工具,與 `wasmedge run` 命令相同。對於目前統一的 `wasmedge` CLI,請參考下方的 [API](#unified-cli)。
...
請注意,此 API 代表舊的 `wasmedgec` CLI 工具,與 `wasmedge compile` 命令相同。對於目前統一的 `wasmedge` CLI,請參考下方的 [API](#unified-cli)。
...
在 Windows 平台上,開發者可使用 `WasmEdge_Driver_ArgvCreate()` 與 `WasmEdge_Driver_ArgvDelete()` API 來轉換並處理 `UTF-8` 命令列引數,或使用 `WasmEdge_Driver_SetConsoleOutputCPtoUTF8()` API 將主控台輸出代碼頁設為 `UTF-8`。
