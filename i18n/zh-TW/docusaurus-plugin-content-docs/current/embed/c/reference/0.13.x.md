...
[WasmEdge C API](https://github.com/WasmEdge/WasmEdge/blob/master/include/api/wasmedge/wasmedge.h) 表示用於存取 `0.13.5` 版本 WasmEdge 執行環境的介面。以下是使用 WasmEdge C API 的指南。

**開發者可參考[這裡升級至 v0.14.0](upgrade_to_0.14.0)。**
...
WasmEdge C API 自 `0.11.0` 釋出版本起導入 SONAME 與 SOVERSION,用以呈現不同 C API 版本之間的相容性。
...
| 自 0.13.0 起 | libwasmedge.so | libwasmedge.so.0 | libwasmedge.so.0.0.3 |
...
開發者亦可使用 `WasmEdge_LogOff()` API 停用所有日誌記錄。
...
設定上下文 `WasmEdge_ConfigureContext` 管理 `Loader`、`Validator`、`Executor`、`VM` 與 `Compiler` 上下文的設定。開發者可調整有關提案、VM 主機預先註冊(例如 `WASI`)與 AOT 編譯器選項的設定,然後將 `Configure` 上下文套用到所建立的執行環境上下文。
...
2. 主機註冊
   ...
   此設定用於 `VM` 上下文開啟 `WASI` 支援,且僅在 `VM` 上下文中有效。
   ...
   詳情將在 [VM 上下文的預先註冊](#built-in-host-modules-and-plug-in-preregistrations) 中介紹。
...
2. 外掛
   ...
   `VM` 上下文在建立時會自動建立並註冊已載入外掛的模組。此外,若未載入外掛,將會模擬以下主機模組:
   ...
   當 WASM 想要呼叫這些主機函式但未安裝對應外掛時,WasmEdge 會輸出錯誤訊息並回傳錯誤。
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc) 是 WebAssembly 之外的函式,並以匯入的方式傳遞給 WASM 模組。在 WasmEdge 中,主機函式會被組合成具有模組名稱的主機模組,作為 `WasmEdge_ModuleInstanceContext` 物件。詳情請參考 [WasmEdge 執行環境中的主機函式](#host-functions)。

在本章節中,我們展示將主機模組註冊到 `VM` 上下文中的範例。注意,開發者應保證已註冊模組實例的可用性,當該模組實例不再使用時,應將其刪除。
...
5. 列出並取得已註冊的模組
   ...
   要列出並取得 `VM` 上下文中的已註冊模組,除了存取 `VM` 的 `store` 上下文之外,開發者可以使用以下 API。
...
1. 將 `AST module` 實例化並註冊為具名 `Module` 實例
   ...
   如同[註冊主機模組](#host-module-registrations) 或在 `VM` 上下文中[匯入 WASM 模組](#wasm-registrations-and-executions),開發者可以將 `AST module` 上下文實例化為具名 `Module` 實例,並將其註冊到 `Store` 上下文。註冊後,結果 `Module` 實例會以給定的模組名稱匯出到 `Store`,並可在實例化另一個模組時連結。

   `Module` 實例 API 的詳情請參考 [實例](#instances)。`Store` 上下文僅為連結器,用於實例化時搜尋並連結匯出的模組。開發者應在不再使用時刪除輸出的 `Module` 實例。當 `Module` 實例被刪除時,它將自動取消連結所有連結的 `Store` 上下文。
...
5. 非同步呼叫函式
   ...
   如同 [在 VM 中非同步執行 WASM 函式](#asynchronous-execution),開發者亦可透過 `Executor` 上下文 API 非同步呼叫函式。

   在取得 `Function` 實例後,開發者將會透過呼叫 `WasmEdge_ExecutorAsyncInvoke()` API 取得 `Async` 上下文。請參考 [非同步](#async) 章節以了解如何使用此上下文取得結果。
...
[Store](https://webassembly.github.io/spec/core/exec/runtime.html#store) 是執行環境結構,用以呈現可由 WebAssembly 程式操作的所有全域狀態。WasmEdge 中的 `Store` 上下文是一個物件,呈現連結器以在實例化 WASM 模組時提供實例的匯出與匯入。開發者可從 `Store` 上下文取得具名模組,並且應該刪除已註冊到 `Store` 上下文中、不再使用的 `Module` 實例。

當 `Store` 上下文被刪除時,連結的 `Module` 實例會自動取消連結到此 `Store` 上下文。當 `Module` 實例被刪除時,它會自動取消連結到所有連結的 `Store` 上下文。
...
8. 帶有終結器的模組實例中的主機資料
   ...
   除了將主機資料設定到主機函式之外,開發者可將主機資料及其終結器的所有權設定並移轉到 `Module` 實例上下文中。這在實作外掛時可能很有用。
...
WasmEdge 外掛是共享函式庫,提供 WasmEdge 執行環境用以載入並建立主機模組實例。透過外掛,WasmEdge 執行環境可以更輕鬆地擴充。
...
3. 如果 WasmEdge 安裝在系統目錄下(例如 `/usr` 與 `/usr/local`),則為函式庫路徑下的 `./wasmedge/` 目錄。
...
`WasmEdge_Driver_Tool()` API 提供與執行 [`wasmedge run` 指令](../../../start/build-and-run/run.md) 相同的功能。

請注意,此 API 呈現舊的 `wasmedge` CLI 工具,與 `wasmedge run` 指令相同。對於目前統一的 `wasmedge` CLI,請參考 [下方的 API](#unified-cli)。
...
請注意,此 API 呈現舊的 `wasmedgec` CLI 工具,與 `wasmedge compile` 指令相同。對於目前統一的 `wasmedge` CLI,請參考 [下方的 API](#unified-cli)。
...
在 Windows 平台上,開發者可使用 `WasmEdge_Driver_ArgvCreate()` 與 `WasmEdge_Driver_ArgvDelete()` API 轉換並處理 `UTF-8` 命令列引數,或使用 `WasmEdge_Driver_SetConsoleOutputCPtoUTF8()` API 將主控台輸出代碼頁設定為 `UTF-8`。
