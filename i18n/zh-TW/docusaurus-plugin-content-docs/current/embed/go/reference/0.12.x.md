...
**開發者可以參考[此處升級至 v0.13.0](upgrade_to_0.13.0)。**
...
- [WasmEdge VM](#wasmedge-vm)
  - [使用 VM 物件的 WASM 執行範例](#wasm-execution-example-with-vm-object)
  - [建立 VM](#vm-creations)
...
- [WasmEdge 執行環境](#wasmedge-runtime)
  - [逐步執行 WASM 範例](#wasm-execution-example-step-by-step)
  - [Loader](#loader)
  - [Validator](#validator)
  - [Executor](#executor)
...
如果開發者需要 `WasmEdge-go` 的 `TensorFlow` 或 `Image` 擴充,請安裝帶有擴充的 `WasmEdge`:

```bash
curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install.sh | bash -s -- -e tf,image -v 0.12.1
```

請注意,`TensorFlow` 與 `Image` 擴充僅適用於 `Linux` 平台。安裝完成後,開發者可以使用 `source` 命令來更新 include 與連結搜尋路徑。
...
預設情況下,`WasmEdge-go` 只開啟基本執行環境。

`WasmEdge-go` 有以下擴充(僅在 Linux 平台上):

- Tensorflow

  - 此擴充支援 [WasmEdge-tensorflow](https://github.com/second-state/WasmEdge-tensorflow) 中的主機函式。
  - 安裝 `WasmEdge` 時需要 `TensorFlow` 與 `TensorFlow-Lite` 擴充。請使用 `-e tensorflow` 命令安裝 `WasmEdge`。
  - 使用此擴充時,建置時需要 `tensorflow` 標籤:
...
- Tensorflow-Lite
  ...
  - 此擴充僅支援 [WasmEdge-tensorflow](https://github.com/second-state/WasmEdge-tensorflow) 中具有 `TensorFlow-Lite` 的主機函式。
  - 安裝 `WasmEdge` 時需要 `TensorFlow-Lite` 擴充。請使用 `-e tensorflow` 命令安裝 `WasmEdge`。
  - **此標籤無法與 `tensorflow` 標籤一起使用。**
  - 使用此擴充時,建置時需要 `tensorflow` 標籤:
...
- Image
  ...
  - 此擴充支援 [WasmEdge-image](https://github.com/second-state/WasmEdge-image) 中的主機函式。
  - 安裝 `WasmEdge` 時需要 `Image` 擴充。請使用 `-e image` 命令安裝 `WasmEdge`。
  - 使用此擴充時,建置時需要 `image` 標籤:
...
1. 將 `AST` 物件實例化並註冊為命名 `Module` 實例
   ...
   就像 `VM` 物件中[註冊主機模組](#host-module-registrations)或[匯入 WASM 模組](#wasm-registrations-and-executions)一樣,開發者可以透過 `Executor` API 將 `AST` 物件實例化並註冊到 `Store` 上下文中,作為命名的 `Module` 實例。註冊後,產生的 `Module` 實例會以給定的模組名稱匯出,並可在實例化另一個模組時被連結。關於 `Module` 實例 API 的細節,請參考[實例](#instances)。
...
### Store
[Store](https://webassembly.github.io/spec/core/exec/runtime.html#store) 是執行環境結構,用於表示所有可由 WebAssembly 程式操作的全域狀態。WasmEdge 中的 `Store` 物件是用於在實例化 WASM 模組時提供實例匯出與匯入的物件。開發者可以從 `Store` 上下文中取得命名的模組。
...
[主機函式](https://webassembly.github.io/spec/core/exec/runtime.html#syntax-hostfunc)是 WebAssembly 之外的函式,並作為匯入傳遞給 WASM 模組。在 WasmEdge 中,開發者可以建立 `Function`、`Memory`、`Table` 與 `Global` 物件,並將它們加入到 `wasmedge.Module` 物件以註冊到 `VM` 或 `Store`。
