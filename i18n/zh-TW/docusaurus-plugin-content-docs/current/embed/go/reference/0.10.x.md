...
**開發者可以參考[此處升級至 0.11.0](upgrade_to_0.11.0.md)。**
...
- [WasmEdge-go 基礎](#wasmedge-go-basics)
  ...
  - [設定](#configurations)
  - [統計資訊](#statistics)
  - [工具驅動器](#tools-driver-v0101-or-upper-only)
...
- Tensorflow-Lite (僅限 `v0.10.1` 或更新版本)
...
1. Limit
   ...
   對於 thread 提案,`Limit` 結構也支援共用記憶體的描述。(僅限 `v0.10.1` 或更新版本)
...
3. [WASI-NN 提案](https://github.com/WebAssembly/wasi-nn) (僅限 `v0.10.1` 或更新版本)

   開發者可以在 `Configure` 物件中開啟 VM 的 WASI-NN 提案支援。

   > 注意:請確認[相依性與必要條件](../../../start/install.md#wasi-nn-plug-in-with-openvino-backend)已滿足。
...
4. [WASI-Crypto 提案](https://github.com/WebAssembly/wasi-crypto) (僅限 `v0.10.1` 或更新版本)
...
3. 指定的模組實例
   ...
   `wasmedge.NewWasiNNModule()` API 可以建立並初始化 `WASI-NN` 外掛的 `wasi_ephemeral_nn` 模組實例 (僅限 `v0.10.1` 或更新版本)。

   `wasmedge.NewWasiCryptoCommonModule()` API 可以建立並初始化 `WASI-Crypto` 外掛的 `wasi_ephemeral_crypto_common` 模組實例 (僅限 `v0.10.1` 或更新版本)。

   `wasmedge.NewWasiCryptoAsymmetricCommonModule()` API 可以建立並初始化 `WASI-Crypto` 外掛的 `wasi_ephemeral_crypto_asymmetric_common` 模組實例 (僅限 `v0.10.1` 或更新版本)。

   `wasmedge.NewWasiCryptoKxModule()` API 可以建立並初始化 `WASI-Crypto` 外掛的 `wasi_ephemeral_crypto_kx` 模組實例 (僅限 `v0.10.1` 或更新版本)。

   `wasmedge.NewWasiCryptoSignaturesModule()` API 可以建立並初始化 `WASI-Crypto` 外掛的 `wasi_ephemeral_crypto_signatures` 模組實例 (僅限 `v0.10.1` 或更新版本)。

   `wasmedge.NewWasiCryptoSymmetricModule()` API 可以建立並初始化 `WASI-Crypto` 外掛的 `wasi_ephemeral_crypto_symmetric` 模組實例 (僅限 `v0.10.1` 或更新版本)。
   ...
   > 注意:對於 `WASI-NN` 外掛,請確認[相依性與必要條件](../../../start/install.md#wasi-nn-plug-in-with-openvino-backend)已滿足。注意:對於 `WASI-Crypto` 外掛,請確認[相依性與必要條件](../../../start/install.md#wasi-crypto-plug-in)已滿足。建議將 5 個模組全部一起建立並註冊。
