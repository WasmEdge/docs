...
title: 部署 Proxy-Wasm 應用程式
---

# Proxy-Wasm

Proxy-Wasm 是介於代理(如 Envoy 或 MOSN)與 WebAssembly 虛擬機器之間的二進位介面(ABI)。WasmEdge 可作為 Proxy-Wasm 主機的高效能執行環境後端無縫整合。

本教學示範如何使用 [proxy-wasm-cpp-host](https://github.com/proxy-wasm/proxy-wasm-cpp-host) 函式庫建置獨立的 Proxy-Wasm 主機、撰寫一個 WebAssembly 過濾器,並使用 WasmEdge 原生執行它。

## 必備條件

請確保你的系統已安裝以下項目:

1. **WasmEdge:** 版本 `0.14.1` 或更高。(參閱 [安裝 WasmEdge](https://wasmedge.org/docs/start/install/))
2. **建置工具:** C++ 編譯器(`clang`/`gcc`)、`cmake` 與 `ninja`。
3. **WASI-SDK:** 編譯 WebAssembly 外掛時需要(v32 或更新)。

_(macOS 範例:在工作目錄安裝 WASI-SDK-32 並匯出 WASI_SDK_PATH)_:
...
4. **系統函式庫:** OpenSSL 與 Abseil(Google 的 C++ 函式庫)。
...
建立工作目錄並取得所需的 Proxy-Wasm 上游相依項目:
...
建立名為 `plugin` 的目錄。此過濾器掛接於 HTTP 回應階段,記錄請求,並注入自訂的 `x-powered-by: WasmEdge` 標頭。
...
使用 WASI-SDK 編譯 WebAssembly 外掛:

```bash
# 2. 使用 p1 toolchain 設定
...
建立名為 `host` 的目錄。主機會初始化 WasmEdge 虛擬機器、載入已編譯的 WebAssembly 過濾器,並模擬 HTTP 網路流量。
...
執行已編譯的原生主機,並將 WebAssembly 外掛傳入。請確保 WasmEdge 函式庫位於你的路徑中。

```bash
# macOS 請使用 DYLD_LIBRARY_PATH,Linux 請使用 LD_LIBRARY_PATH
LD_LIBRARY_PATH=$HOME/.wasmedge/lib ./build/host/demo ./build/plugin/add_header_filter.wasm
```

## 預期輸出

執行成功後,主機將載入 WebAssembly 過濾器並模擬網路流量。輸出將明確顯示外掛注入了自訂的 x-powered-by: WasmEdge 標頭:
...
恭喜!你已成功使用 WasmEdge 原生編譯並執行 Proxy-Wasm 過濾器。現在你已具備開發高效能 WebAssembly 外掛的基礎,可以在你的雲端原生基礎架構中檢視、路由並安全地變更網路流量。
