...
KubeEdge 是一個開源系統,用以將原生容器化應用程式編排能力擴展至邊緣端的主機。

<!-- prettier-ignore -->
:::note
本示範基於 [crun 的支援](../../deploy/oci-runtime/crun.md)
:::

## 1. 設定雲端側(KubeEdge 主節點)
...
請參閱 [CRI-O 安裝說明](https://github.com/cri-o/cri-o/blob/main/install.md#install-packaged-versions-of-cri-o)。
...
### 為 K8s 安裝並使用 kubeadm 建立叢集

請參閱[使用 kubeadm 建立叢集](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/create-cluster-kubeadm/)。
...
若要讓 kubectl 適用於你的非 root 使用者,執行下列命令,這些也是 kubeadm init 輸出的一部分:
...
請參閱[使用 Keadm 部署](https://kubeedge.io/en/docs/setup/keadm/)。

1. 至少必須正確設定一個 kubeconfig 或 master,以便能用來驗證 k8s 叢集的版本與其他資訊。
2. 請確保邊緣節點能使用雲端節點的本機 IP 連線至雲端節點,或者你需要使用 --advertise-address 旗標指定雲端節點的公開 IP。
3. --advertise-address(僅自 1.3 版以後有效)是雲端側暴露的位址(會加入 CloudCore 憑證的 SAN);預設值為本機 IP。
...
## 2. 設定邊緣側(KubeEdge 工作節點)

你可以使用 CRI-O 的 [install.sh] 指令碼在 Ubuntu 20.04 上安裝 CRI-O 與 `crun`。
...
在雲端側執行 keadm gettoken 會回傳 token,加入邊緣節點時將會用到。
...
請參閱[使用 CRI 設定不同容器執行環境](https://kubeedge.io/en/docs/advanced/cri/#cri-o)與[使用 Keadm 部署](https://kubeedge.io/en/docs/setup/keadm/)。
...
## 3. 啟用 kubectl logs 功能

在部署 metrics-server 之前,必須啟用 kubectl logs 功能,請[參閱此處](https://kubeedge.io/en/docs/setup/keadm/#enable-kubectl-logs-feature)。

## 4. 執行一個簡單的 WebAssembly 應用程式

我們可以在 Kubernetes 叢集中執行來自 Docker Hub 的基於 WebAssembly 的映像檔。
...
pod 的 WebAssembly 應用程式成功部署到邊緣節點。
